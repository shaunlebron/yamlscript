...
public:
    ...
    bool testeq(std::vector<ievt::DataType> const& actual, csubstr parsed_source, csubstr arena) const
    {
        ...
        for(size_t ia = 0, ie = 0; ie < num_events_expected; ++ie)
        {
            ...
            csubstr actualbuf = c4::yml::extra::ievt::mkstring(actual[ia] & ievt::MASK, actualbuf_);
            csubstr expectedbuf = c4::yml::extra::ievt::mkstring(evt[ie].flags & ievt::MASK, expectedbuf_);
